<script lang="ts">
  ...
  import { exampleFeaturedAndMates } from "$lib/character-builds";
  ...
  type DemoExampleTeam = {
    id: string;
    members: string[];
    high: boolean;
    stats: { key: string; label: string; value: string; iconKey: string }[];
  };

  let examplePickId = $state("");
  let exampleMenuOpen = $state(false);
  let examplePickerPool = $derived($charactersOwned.slice(0, 12));
  let examplePickerMap = $derived(
    new Map(examplePickerPool.map((c) => [c.name_id ?? c.name, c])),
  );
  let examplePickerFeaturedId = $derived(
    examplePickerPool[0]?.name_id ?? examplePickerPool[0]?.name ?? "",
  );
  let demoExampleTeams = $derived.by((): DemoExampleTeam[] => {
    const pool = examplePickerPool;
    if (pool.length < 7) return [];
    const id = (c: (typeof pool)[number]) => c.name_id ?? c.name;
    const featured = id(pool[0]!);
    const teams: DemoExampleTeam[] = [
      {
        id: "ex-a",
        members: [featured, id(pool[1]!), id(pool[2]!), id(pool[3]!)],
        high: true,
        stats: [
          { key: "atk", label: "ATK", value: "2,184", iconKey: "atk" },
          {
            key: "critRate",
            label: "CRIT Rate",
            value: "72.4%",
      ...
      {
        id: "ex-b",
        members: [featured, id(pool[4]!), id(pool[5]!), id(pool[2]!)],
        high: false,
        stats: [
          { key: "atk", label: "ATK", value: "1,942", iconKey: "atk" },
          {
            key: "enerRech",
      ...
      {
        id: "ex-c",
        members: [featured, id(pool[6]!), id(pool[3]!), id(pool[5]!)],
        high: false,
        stats: [
          {
            key: "eleMas",
          ...
          { key: "atk", label: "ATK", value: "1,510", iconKey: "atk" },
          {
            key: "enerRech",
            label: "Energy Recharge",
            value: "132.0%",
  ...
  let activeDemoExample = $derived.by(() => {
    const list = demoExampleTeams;
    if (!list.length) return null;
    return list.find((t) => t.id === examplePickId) ?? list[0] ?? null;
  });
  let demoExampleAlts = $derived(
    demoExampleTeams.filter((t) => t.id !== activeDemoExample?.id),
  );
...
<PageShell class="gap-10">
  ...
  <section class="gallery-section" id="build-example-picker">
    <div class="section-head">
      <p class="concept-kicker">Character builds · example picker</p>
      <h2>One team + gear for other rows</h2>
      <p>
        Default shows the selected lineup. Gear reveals other examples as fully
        expanded mate-only rows (no overlap), aligned under the mate columns.
      </p>
    </div>

    {#if demoExampleTeams.length === 0 || !activeDemoExample}
      <Surface>
        <p class="token-meta">
          Need at least 7 roster characters hydrated to demo the picker.
        </p>
      </Surface>
    {:else}
      {@const featuredId = examplePickerFeaturedId}
      {@const activeSplit = exampleFeaturedAndMates(
        activeDemoExample.members,
        featuredId,
      )}
      <div class="example-picker-demo">
        <div class="example-picker-primary">
          <div class="example-picker-main">
            ...
            >
              <div class="example-picker-slot example-picker-slot--featured">
                {#if activeSplit.featured}
                  {@const featured = examplePickerMap.get(activeSplit.featured)}
                  {#if featured}
                    <CharacterIcon
                      character={featured}
                      iconStyle="tcg"
              ...
              {#each activeSplit.mates as mateId, i (mateId ?? `active-empty-${i}`)}
                <div class="example-picker-slot">
                  {#if mateId}
                    {@const mate = examplePickerMap.get(mateId)}
                    {#if mate}
                      <CharacterIcon
                        character={mate}
                        iconStyle="tcg"
                        ...
                        >
                          <div class="example-picker-grid">
                            ...
                            <div class="example-picker-mate-row">
                              {#each split.mates as mateId, i (mateId ?? `alt-${team.id}-${i}`)}
                                <div class="example-picker-slot">
                                  {#if mateId}
                                    {@const mate =
                                      examplePickerMap.get(mateId)}
                                    {#if mate}
                                      <CharacterIcon
                                        character={mate}
                                        iconStyle="tcg"
          ...
          {#if demoExampleAlts.length > 0}
            ...
            >
              <IconCog size={18} strokeWidth={2} />
            </button>
          {/if}
        </div>

        <div class="example-picker-sheet">
          {#if activeDemoExample.high}
            <p class="meta-sub">High artifacts</p>
          {/if}
          <div class="example-picker-stats">
            {#each activeDemoExample.stats as row (row.key)}
              <StatRow
                label={row.label}
                value={row.value}
...
<style>
  ...
  /* Height clip slide; rows stay fully expanded (no overlap) when open. */
